  ...
  return (
    <main className="relative flex min-h-screen flex-col items-center justify-center text-white overflow-hidden">
      ...
      <div className="absolute top-4 right-4 z-10">
        <Link href="/login">
          <div className="items-center bg-white/10 p-2 text-white hover:bg-indigo-400 hover:text-black rounded-md text-sm py-2 px-4">
            Login
          </div>
        </Link>
      </div>

      <div className="container flex flex-col items-center justify-center gap-8 px-4 py-8 z-10">
        ...
        <div className="w-full max-w-md flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            {messages.map((msg, index) => (
              <div key={index} className="p-3 bg-white/10 rounded-lg text-sm">
                {msg.content}
              </div>
            ))}
          </div>
          ...
          <button
            ...
            className="items-center bg-white/10 p-3 text-white hover:bg-indigo-400 hover:text-black rounded-md"
          >
            Submit
          </button>
        </div>
